...
const Navbar = () => {
    ...
    return (
        <div className='flex justify-between items-center md:py-5 py-2 px-2 text-sm rounded-sm border-slate-500 border-solid border-b-4 dark:bg-dar dark:text-white '>
            {/* <h1 onClick={()=>navigate('/')} className='cursor-pointer'>Doctor</h1> */}
            <div onClick={() => navigate('/')} className="md:mb-0 cursor-pointer flex items-center">
                <div className="flex items-center justify-center">
                    <img src={assets.logo1} className="md:h-8 h-6 me-3 mt-1 border-[2px] border-teal-600 rounded-b-full" alt="FlowBite Logo" />
                    <span className="self-center md:text-2xl text-lg font-semibold whitespace-nowrap bg-gradient-to-r from-blue-600 to-red-600 bg-clip-text text-transparent">CareBridge</span>
                </div>
            </div>
            ...
            <div className='flex items-center gap-4'>
                {
                    token && userData ?
                        <div className='flex items-center gap-2 cursor-pointer group relative'>
                            <div className='flex items-center gap-2 bg-slate-100 dark:bg-dar2 px-2 py-1 rounded-full'>
                                <img className='rounded-full w-8 h-8' src={userData.image} alt="" />
                                <p className='text-sm hidden md:block '>{userData.name}</p>
                ...
                <img onClick={() => setShowMenu(true)} className='md:w-6 w-4 md:hidden' src={assets.menu_icon} alt="" />
                {/* Mobile Menu */}
